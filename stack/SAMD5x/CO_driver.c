        ...
        uint16_t                CANbitRate)
{
	...
	//can_async_register_callback(CANmodule->descr, CAN_ASYNC_IRQ_CB, (FUNC_PTR)CO_ErrCallback);
...
void CO_ErrCallback(struct can_async_descriptor *descr, enum can_async_interrupt_type type)
{
	switch (type) {
		case CAN_IRQ_BO:
			printf("CAN IRQ: Bus Off State\r\n");
			break;

		case CAN_IRQ_DO:
			printf("CAN IRQ: Data Overrun\r\n");
			break;

		case CAN_IRQ_EA:
			printf("CAN IRQ: Error Active State\r\n");
			break;

		case CAN_IRQ_EP:
			printf("CAN IRQ: Error Passive State\r\n");
			break;

		case CAN_IRQ_EW:
			printf("CAN IRQ: Error Warning\r\n");
			break;
